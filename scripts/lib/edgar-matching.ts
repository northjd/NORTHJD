 ...
 * Adyen.
 */
export function parseCompanyName(raw: string): ParsedName {
  const words = raw
    .normalize('NFD')
    .replace(/[̀-ͯ]/g, '')
    .replace(/['’]/g, '')
...
export function resolveFiler(
  ...
  const { key, forms } = parseCompanyName(entityName);
  if (key.length < 2) return { filer: null, reason: 'none' };
  const ticker = normaliseTicker(entityTicker ?? '');

  const sameName = (index.byName.get(key) ?? []).filter((f) => formsAgree(forms, f.forms));
