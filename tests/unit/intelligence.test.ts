...
const doc = (over: Partial<ClusterableDocument>): ClusterableDocument => ({
  documentId: 'd', title: 't', summary: 's',
  publishedAt: new Date('2026-08-29T09:00:00Z'), eventAt: null,
  entityIds: [], fingerprint: 'f', sourceId: 's1',
  isFirstParty: false, isIndependent: true,
  ...over,
});
...
describe('claim extraction', () => {
  it('classifies a completed action as FACT and a commitment as FORECAST', () => {
    const claims = extractClaims({
      ...
      title: 'Expansion', perspective: 'FIRST_PARTY_COMPANY', sourceType: 'official_newsroom',
    });
  ...
  it('marks hedged language as interpretation, not fact', () => {
    const claims = extractClaims({
      normalizedText: 'Analysts suggest the initiative could improve availability across the estate.',
      title: 'Analysis', perspective: 'INDEPENDENT_BUSINESS_MEDIA', sourceType: 'independent_news',
    });
    expect(claims[0]?.claimType).toBe('INTERPRETATION');
  });

  it('produces offsets that select the claim text exactly', () => {
    const text = 'H&M announced a partnership with a technology provider today. It covers twelve markets.';
    const claims = extractClaims({
      normalizedText: text, title: 'x', perspective: 'FIRST_PARTY_COMPANY', sourceType: 'official_newsroom',
    });
  ...
  it('skips feed syndication footers', () => {
    const claims = extractClaims({
      normalizedText: 'The post Meet the winners appeared first on H&M Group. The programme selected five winners this year.',
      title: 'x', perspective: 'FIRST_PARTY_COMPANY', sourceType: 'official_newsroom',
    });
...
describe('evidence strength', () => {
  ...
  it('treats a regulator as primary evidence', () => {
    expect(deriveEvidenceStrength('REGULATOR', 'regulator', false)).toBe('UNQUANTIFIED_PRIMARY_EVIDENCE');
  });
});

describe('case maturity — the hype filter', () => {
  it('does not promote a self-reported outcome to validated', () => {
    expect(classifyCaseMaturity('The company reported an 18% reduction in markdown rate.', false).maturity).toBe(
      'QUANTIFIED_BUSINESS_IMPACT',
    );
  });

  it('promotes it only with independent corroboration', () => {
    expect(classifyCaseMaturity('The company reported an 18% reduction in markdown rate.', true).maturity).toBe(
  ...
  it('detects a reversal', () => {
    expect(classifyCaseMaturity('The company has discontinued the programme.', false).maturity).toBe(
      'DISCONTINUED_OR_REVERSED',
    );
  });

  it('defaults to announced when no scope is stated', () => {
    expect(classifyCaseMaturity('The company announced a new initiative.', false).maturity).toBe('ANNOUNCED');
  });

  it('always explains itself', () => {
    expect(classifyCaseMaturity('A pilot is under way.', false).rationale.length).toBeGreaterThan(10);
  });
});
...
describe('event type and levers', () => {
  it('reads the event type from the headline', () => {
    ...
    expect(classifyEventType('Acme appoints new chief financial officer', '')).toBe('leadership_change');
...
describe('strategic impact', () => {
  it('ranks a validated outcome above an announcement', () => {
    const announced = scoreStrategicImpact({
      eventType: 'partnership', maturity: 'ANNOUNCED', quantified: false,
      entityCount: 1, independentSourceCount: 0,
    });
    const validated = scoreStrategicImpact({
      eventType: 'partnership', maturity: 'INDEPENDENTLY_VALIDATED_IMPACT', quantified: true,
      entityCount: 2, independentSourceCount: 2,
    });
...
describe('entity resolution', () => {
  const meta = [{
    entityId: 'meta', name: 'Meta', officialDomain: 'about.fb.com',
    aliases: [
  ...
  it('refuses an ambiguous alias on its own', () => {
    expect(resolveEntities(meta, { title: 'Improving meta descriptions', body: 'the meta tag' })).toHaveLength(0);
  });

  it('accepts it when an unambiguous alias co-occurs', () => {
    const found = resolveEntities(meta, { title: 'Meta Platforms ships a model', body: 'Meta said so.' });
  ...
  it('always resolves the subject of a first-party source', () => {
    const found = resolveEntities(meta, {
      title: 'An update', body: 'No company name here.', sourceSubjectEntityId: 'meta',
    });
...
describe('event clustering', () => {
  it('merges two reports of one deal', () => {
    const a = doc({ documentId: 'a', entityIds: ['x', 'y'], fingerprint: 'a',
      title: 'Meridian and Halden AI announce partnership on demand forecasting',
      summary: 'Meridian and Halden announced a partnership on demand forecasting with a proof of concept in 40 stores.' });
    const b = doc({ documentId: 'b', entityIds: ['x', 'y'], fingerprint: 'b', sourceId: 's2',
      title: 'Halden AI signs Meridian as anchor customer',
      summary: 'Halden signed Meridian as anchor customer for demand forecasting, starting with a proof of concept in 40 stores.' });
    expect(describesSameEvent(a, b)).toBe(true);
  });

  it('keeps different companies apart despite identical wording', () => {
    const a = doc({ documentId: 'a', entityIds: ['x'], fingerprint: 'a', title: 'A announces partnership' });
    const b = doc({ documentId: 'b', entityIds: ['y'], fingerprint: 'b', title: 'B announces partnership' });
    expect(describesSameEvent(a, b)).toBe(false);
  });

  it('keeps the same story apart when months separate the reports', () => {
    const a = doc({ documentId: 'a', entityIds: ['x'], fingerprint: 'a', title: 'X announces platform rollout' });
    const b = doc({ documentId: 'b', entityIds: ['x'], fingerprint: 'b',
      title: 'X announces platform rollout', publishedAt: new Date('2026-04-01T00:00:00Z') });
    expect(describesSameEvent(a, b)).toBe(false);
  });
});
...
describe('corrections and contradictions', () => {
  ...
  it('does not invent a conflict between unrelated statements', () => {
    expect(detectContradiction('H&M opened a store in Berlin.', 'NVIDIA released a new GPU.')).toBeNull();
  });
});

describe('taxonomy matching', () => {
  it('matches whole words only', () => {
    const terms = [{ kind: 'topic' as const, slug: 'artificial-intelligence', name: 'AI', aliases: [] }];
...
describe('marketing neutralisation', () => {
  it('strips unsupported superlatives from first-party headlines', () => {
    expect(neutralizeMarketingHeadline('Revolutionary best-in-class AI platform launched', true)).toBe(
