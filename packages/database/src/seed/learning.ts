    ...
    units: [
      {
        ...
        structuredModel: [
          { heading: 'The margin equation', points: ['Cost price is fixed at order commitment, months ahead', 'Realised price is what the customer actually paid after markdown', 'Gross margin is the gap, less shrink and returns', 'Working capital is consumed the whole time the stock is unsold'] },
          { heading: 'Where it goes wrong', points: ['Over-buying: too many units committed pre-season', 'Mis-allocation: right units, wrong stores', 'Late reaction: markdown taken too late to clear at a reasonable price', 'Returns: a digital order that comes back cost money twice'] },
          { heading: 'The structural constraint', points: ['Production lead times of 8–40 weeks force pre-season commitment', 'Shortening lead time reduces the size of the bet, which is why speed is strategic rather than operational'] },
        ],
        keyTerms: [
          { term: 'Open-to-buy', definition: 'The budget still available for future purchases in a plan period. The control that stops merchandising over-committing.' },
          { term: 'Markdown', definition: 'A permanent price reduction taken to clear stock. Distinct from a promotion, which is temporary.' },
          { term: 'Sell-through', definition: 'The share of a buy that has sold, usually measured against plan at a point in the season.' },
        ],
        coreMetricSlugs: ['full-price-sell-through', 'markdown-rate', 'gross-margin', 'sell-through-rate'],
        ...
        sourceRefs: [{ label: 'H&M Group annual reporting', url: 'https://hmgroup.com/investors/' }],
        estimatedMinutes: 6,
        knowledgeCheck: {
          question: 'A retailer grows revenue 8% while markdown rate rises from 22% to 29% and full-price sell-through falls. What is the most likely explanation?',
          options: [
      ...
      },
      {
        ...
        objective: 'Identify the small number of merchandising decisions that set the financial outcome of a season.',
        explanation:
        ...
        structuredModel: [
          { heading: 'The four decisions', points: ['Pre-season commitment vs. open-to-buy reserve', 'Depth vs. breadth within the range', 'Store clustering and size curves', 'First markdown timing and depth'] },
          { heading: 'Who decides what', points: ['Buying owns cost price and supplier relationship', 'Merchandising owns the financial plan and open-to-buy', 'Allocation owns unit placement', 'Retail operations owns execution and can quietly override all of it'] },
          { heading: 'Why AI planning stalls', points: ['The model produces a recommendation; the org still requires a human sign-off', 'Overrides are rarely tracked, so nobody knows whether the model or the merchandiser was right', 'Without an override-tracking loop there is no way to build trust or to improve'] },
        ],
        keyTerms: [
          { term: 'Store clustering', definition: 'Grouping stores by demand profile so allocation rules can differ meaningfully between them.' },
          { term: 'Size curve', definition: 'The distribution of sizes allocated to a location, ideally derived from that location\'s historical sales rather than a national average.' },
          { term: 'Override rate', definition: 'How often a human changes a system recommendation. The single most informative metric about an AI planning deployment.' },
        ],
        ...
        estimatedMinutes: 8,
        knowledgeCheck: {
          question: 'A retailer deploys a demand forecasting model. Accuracy improves materially, but markdown does not fall. What should be examined first?',
          options: [
      ...
      },
      {
        ...
        objective: 'Analyse the organisational changes required before AI-enabled planning delivers measured margin benefit.',
        explanation:
        ...
        structuredModel: [
          { heading: 'What actually changes', points: ['Decision cadence: fortnightly → daily or twice-weekly', 'Decision rights: advisory vs. binding recommendations, made explicit', 'Performance definition: judged on outcome, not on personal call quality', 'Exception handling: which cases still route to a human, and why'] },
          { heading: 'Measurement design', points: ['Hold-out stores or categories to establish a counterfactual', 'Override tracking with outcome attribution', 'Baseline defined and frozen before deployment — not reconstructed afterwards', 'A full seasonal cycle before claiming a result'] },
          { heading: 'Reading a vendor claim', points: ['Was the baseline defined before or after the deployment?', 'Is the comparison like-for-like, or against an anomalous prior period?', 'Is the scope the whole estate or the best-performing subset?', 'Has anyone outside the company verified it?'] },
        ],
        keyTerms: [
          { term: 'Counterfactual', definition: 'What would have happened without the intervention. Without a hold-out group it is an assumption, not a measurement.' },
          { term: 'Binding recommendation', definition: 'A system output that executes unless a human actively intervenes, as opposed to one that waits for approval.' },
          { term: 'Baseline drift', definition: 'Quietly re-defining the comparison period so results look better. The most common flaw in self-reported outcome claims.' },
        ],
        ...
        estimatedMinutes: 11,
        knowledgeCheck: {
          question: 'A company reports an 18% markdown reduction after a group-wide AI allocation rollout, with no hold-out group and no published baseline methodology. How should this be classified?',
          options: [
    ...
    units: [
      {
        ...
        objective: 'Distinguish the stages of an AI initiative and identify which stage a given announcement describes.',
        explanation:
        ...
        structuredModel: [
          { heading: 'The ladder', points: ['Announced — intent stated, nothing built', 'Concept — being explored or prototyped', 'Pilot — running in a bounded scope to learn', 'Limited deployment — live in selected locations or units', 'Scaled deployment — live across the organisation', 'Quantified impact — a measured outcome is claimed', 'Independently validated — someone outside confirms the outcome', 'Discontinued — stopped or reversed'] },
          { heading: 'Evidence required at each step', points: ['Announcement: a press release is sufficient evidence of the announcement itself', 'Deployment: a stated scope — sites, users, transactions', 'Impact: a metric, a baseline, a period and a method', 'Validation: a source that is not the party benefiting'] },
        ],
        keyTerms: [
          { term: 'Pilot', definition: 'A bounded deployment whose purpose is learning, not value. Success is a decision, not a return.' },
          { term: 'Self-reported', definition: 'Stated by the party with an interest in the result. Not false by default; not independent either.' },
          { term: 'Production', definition: 'Running as part of normal operations, with the support and reliability obligations that implies.' },
        ],
        ...
        sourceRefs: [{ label: 'NIST AI Risk Management Framework', url: 'https://www.nist.gov/itl/ai-risk-management-framework' }],
        estimatedMinutes: 6,
        knowledgeCheck: {
          question: 'A vendor announces a "strategic partnership to transform demand forecasting", with a proof of concept in 40 stores in Q4. What maturity is this?',
          options: ['Scaled deployment', 'Quantified business impact', 'Pilot', 'Independently validated impact'],
          correctIndex: 2,
          explanation:
      ...
      },
      {
        ...
        position: 2,
        conceptSlug: 'ai-deployment-maturity',
        objective: 'Explain the recurring non-technical constraints on scaling AI, and what to ask about each.',
        explanation:
        ...
        structuredModel: [
          { heading: 'The four constraints', points: ['Data: pilot-grade data quality does not survive daily operation', 'Decision rights: advisory outputs produce no change in behaviour', 'Evaluation: without a task suite there is no basis for trusting the system', 'Unit economics: cost per inference at full volume may not clear the value bar'] },
          { heading: 'Diagnostic questions', points: ['What would have to be true to run this without human approval?', 'Who is accountable when the system is wrong?', 'What is the evaluation suite, and who owns it?', 'What is the cost per decision at full volume?'] },
        ],
        keyTerms: [
          { term: 'Evaluation suite', definition: 'A held-out set of representative tasks with defined pass criteria. The basis for any defensible claim of fitness.' },
          { term: 'Human in the loop', definition: 'A design where a person approves or corrects output. Often a sensible control; also often the reason no efficiency is realised.' },
          { term: 'Unit economics', definition: 'Cost and value per decision. Determines which use cases are viable independent of model quality.' },
        ],
...
export const EXTRA_CONCEPTS = [
  { slug: 'fashion-economics', name: 'Fashion retail economics', kind: 'industry_concept', industrySlug: 'fashion-apparel', summary: 'How a fashion retailer earns and loses gross margin.' },
  { slug: 'ai-deployment-maturity', name: 'AI deployment maturity', kind: 'industry_concept', industrySlug: 'technology-ai', summary: 'The ladder from announcement through deployment to independently validated impact.' },
];
