...
const FACT_VERBS = [
  'announced', 'launched', 'introduced', 'unveiled', 'acquired', 'purchased', 'sold',
  'signed', 'agreed', 'completed', 'closed', 'opened', 'appointed', 'named', 'hired',
  'reported', 'posted', 'published', 'released', 'filed', 'partnered', 'invested',
  'raised', 'secured', 'awarded', 'selected', 'deployed', 'rolled out', 'implemented',
  'expanded', 'entered', 'exited', 'discontinued', 'shut', 'cut', 'added', 'began',
  'started', 'joined', 'confirmed', 'stated', 'said', 'told', 'disclosed', 'approved',
];

const FORECAST_MARKERS = [
  'will ', 'plans to', 'aims to', 'intends to', 'expects to', 'expects that', 'targets',
  'is targeting', 'forecast', 'outlook', 'guidance', 'projected', 'by 2027', 'by 2028',
  'by 2029', 'by 2030', 'in the coming', 'over the next', 'is set to', 'due to launch',
  'scheduled to',
];

const INTERPRETATION_MARKERS = [
  'could ', 'may ', 'might ', 'suggests', 'indicates', 'appears to', 'seems to',
  'believes', 'is seen as', 'analysts', 'potentially', 'likely to', 'observers',
  'is expected to be', 'arguably', 'reportedly', 'rumoured', 'rumored',
];

/** Feed and page furniture that is not editorial content. */
const BOILERPLATE = [
  'cookie', 'privacy policy', 'terms of use', 'all rights reserved', 'read more',
  'subscribe', 'newsletter', 'follow us', 'share this', 'click here', 'sign up',
  'javascript', 'enable your browser', 'advertisement', 'related articles',
  'copyright ', 'contact us', 'log in', 'accept all',
  ...
  'appeared first on', 'the post ', 'continue reading', 'view the full',
  'this post was', 'originally published', 'image credit', 'photo:',
];

const SUPERLATIVES = [
  'world-leading', 'world leading', 'best-in-class', 'best in class', 'revolutionary',
  'game-changing', 'game changing', 'unparalleled', 'cutting-edge', 'seamlessly',
  'industry-leading', 'industry leading', 'transformative', 'unmatched', 'first-ever',
];
...
export function deriveEvidenceStrength(
  ...
  const attested = ['regulatory_filing', 'annual_report', 'earnings_release', 'earnings_transcript'];

  if (perspective === 'REGULATOR' || perspective === 'PUBLIC_INSTITUTION' || attested.includes(sourceType)) {
